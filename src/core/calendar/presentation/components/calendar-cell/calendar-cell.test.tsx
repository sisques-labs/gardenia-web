...
describe('CalendarCell — null slot', () => {
  it('renders an aria-hidden placeholder when day is null', () => {
    const { container } = render(
      <CalendarCell day={null} isToday={false} isSelected={false} onSelect={vi.fn()} />
    );
  ...
  it('does not render a button when day is null', () => {
    render(<CalendarCell day={null} isToday={false} isSelected={false} onSelect={vi.fn()} />);
...
describe('CalendarCell — day cell', () => {
  it('renders the day number', () => {
    render(<CalendarCell day={15} isToday={false} isSelected={false} onSelect={vi.fn()} />);
    expect(screen.getByText('15')).toBeInTheDocument();
  });

  it('applies today class when isToday is true', () => {
    const { container } = render(
      <CalendarCell day={15} isToday={true} isSelected={false} onSelect={vi.fn()} />
    );
  ...
  it('does not apply today class when isToday is false', () => {
    const { container } = render(
      <CalendarCell day={15} isToday={false} isSelected={false} onSelect={vi.fn()} />
    );
    expect(container.firstChild).not.toHaveClass('is-today');
  });

  it('sets aria-selected=true when isSelected', () => {
    render(<CalendarCell day={15} isToday={false} isSelected={true} onSelect={vi.fn()} />);
  ...
  it('calls onSelect with the day number on click', async () => {
    ...
    render(<CalendarCell day={7} isToday={false} isSelected={false} onSelect={onSelect} />);
